...
--I didn't sort by SalesOrderHeader.SubTotal because a sales order can contain multiple items.
...
--A) By quantity of items sold to each city.
...
--B) By dollar amount sold (quantity * unit price).
SELECT Address.City,
  SUM(SalesOrderDetail.OrderQty * (SalesOrderDetail.UnitPrice - SalesOrderDetail.UnitPriceDiscount)) AS total_sales
FROM ProductCategory JOIN Product ON ProductCategory.ProductCategoryID = Product.ProductCategoryID
  JOIN SalesOrderDetail ON Product.ProductID = SalesOrderDetail.ProductID
...
--C) By percentage of quantity sold to each city represented as a percentage.
--Would use CREATE VIEW AS
